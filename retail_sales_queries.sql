...
FROM
    retail_sales;
--
-- Check the number of sales in each categories
SELECT
    category, COUNT(total_sale)
FROM
    retail_sales
GROUP BY category;
--
...
SELECT
	category, gender, COUNT(total_sale)
FROM
    retail_sales
...
SELECT
	category,
	gender,
	COUNT(transactions_id) AS transactions_made
FROM
	retail_sales
GROUP BY category, gender
ORDER BY category;
...
-- Q9: Identify the top 5 customers based on the highest total sales amount.
SELECT
	customer_id, SUM(total_sale) AS total_sale_by_cus
FROM
	retail_sales
...
-- Q10: Count the number of unique customers who made purchases in each category.
SELECT
    category, COUNT(DISTINCT(customer_id)) AS cus_num
FROM
    retail_sales
GROUP BY category;
--
